...
\begin{frame}
  ...
  \begin{columns}
     ...
     \only<2->{
       \vspace{1ex}
       \bluetext{If you have you have answered at least once ``YES'', you're certainly interested in the topic of this talk!}\\
     }
     \only<3>{
       \vspace{1ex}
       \bluetext{\bf If your interest still remains after the talk, do not hesitate to contact us :)}\\
     }
...
\begin{frame}
  ...
  \begin{columns}[c]
    ...
    \column{.5\textwidth}
    \begin{center}
      \includegraphics[width=0.9\linewidth]{figures/HL-LHC-tt.png}\\
     \end{center}
  \end{columns}
  }
  ...
  \begin{itemize}
    ...
    \item Hadron collider with $E_{\mathrm{cms}}$ at least 100 TeV at CERN as a longer term
  \end{itemize}
  }
\end{frame}  
...
\begin{frame}
  ...
  \only<1->{
    ...
    \column{.4\textwidth}
    \includegraphics[width=1.0\linewidth]{figures/zprime-search-fcchh.png}\\
...
\begin{frame}
  ...
  \bluetext{Highlights from the physics programme}
    \begin{itemize}
      \item Higgs boson couplings, Higgs boson width
  ...
  \bluetext{Advantages of $e^+e^-$ colliders}
     ...
     \item Higgs factories provide model-independent measurements
...
\begin{frame}
  ...
  \only<2>{
  A Higgs factory is needed (BR untagged w/o assumptions!), even if the ultimate goal is the hadron-hadron collider.\\
  }
\end{frame}
...
\begin{frame}
   ...
   \end{center}  
     FCC-ee complete vacuum arc half-cell mock up\\
     \small{
   ...
   \column{.5\textwidth}
     ...
     \end{center}
     400 MHz SRF cryomodule\\
     Prototype multi-cell cavities for FCC $ZH$ operation\\
